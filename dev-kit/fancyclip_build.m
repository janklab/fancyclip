...
reporoot = fancyclip.globals.distroot;
origDir = pwd;
RAII.cd = onCleanup(@() cd(origDir));
cd(reporoot)

fprintf('Building fancyclip...\n')

cfgFile = fullfile(reporoot, '.mlproject.json');
config = jsondecode(fancyclip.internal.util.readtext(cfgFile));
