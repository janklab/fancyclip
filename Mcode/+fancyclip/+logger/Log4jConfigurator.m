classdef Log4jConfigurator
    ...
    % Examples:
    %
    % fancyclip.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % fancyclip.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % fancyclip.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % fancyclip.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % fancyclip.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % fancyclip.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % fancyclip.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = fancyclip.logger.Log4jConfigurator.getLog4jLevel(levelName);
