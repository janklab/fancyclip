classdef Logger
  ...
  % fancyclip.logger logging happens.
  %
  ...
  % arguments.
  %    fancyclip.logger.info(format, varargin)
  ...
  % message.
  %    fancyclip.logger.warn(exception, format, varargin)
  %
  % See also:
  % fancyclip.logger.error
  % fancyclip.logger.warn
  % fancyclip.logger.info
  % fancyclip.logger.debug
  % fancyclip.logger.trace
  %
  % Examples:
  %
  % log = fancyclip.logger.Logger.getLogger('foo.bar.FooBar');
  ...
  methods (Static)
    function out = getLogger(identifier)
      ...
      out = fancyclip.logger.Logger(jLogger);
    end
  end
