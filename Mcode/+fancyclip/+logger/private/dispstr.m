...
if ~ismatrix(x)
    out = sprintf('%s %s', fancyclip.logger.internal.Dispstr.size2str(size(x)), class(x));
elseif isempty(x)
    ...
    elseif isnumeric(x) && isequal(size(x), [0 0])
        ...
        out = sprintf('Empty %s %s', fancyclip.logger.internal.Dispstr.size2str(size(x)), class(x));
    end
elseif isnumeric(x)
    if isscalar(x)
        out = num2str(x);
