classdef DispstrImpl
  ...
  methods (Static)
    ...
    function out = mycombvec(vecs)
      ...
      switch numel(vecs)
        ...
        otherwise
          a = vecs{1}(:);
          rest = vecs(2:end);
          rest_combs = fancyclip.internal.DispstrImpl.mycombvec(rest);
    ...
    function out = prettyprintArray(strs)
      ...
      if ismatrix(strs)
        out = fancyclip.internal.DispstrImpl.prettyprintMatrix(strs);
      else
        sz = size(strs);
        high_sz = sz(3:end);
        high_ixs = {};
        ...
        page_ixs = fancyclip.internal.DispstrImpl.mycombvec(high_ixs);
        chunks = {};
        for i_page = 1:size(page_ixs, 1)
          page_ix = page_ixs(i_page,:);
          chunks{end+1} = sprintf('(:,:,%s) = ', ...
            strjoin(fancyclip.internal.DispstrImpl.num2cellstr(page_ix), ',')); %#ok<*AGROW>
          page_ix_cell = num2cell(page_ix);
          page_strs = strs(:,:,page_ix_cell{:});
          chunks{end+1} = fancyclip.internal.DispstrImpl.prettyprintMatrix(page_strs);
        end
        out = strjoin(chunks, '\n');
      end
      if nargout == 0
    ...
    function out = prettyprintMatrix(strs)
      ...
      widths = max(lens, 1);
      formats = fancyclip.internal.DispstrImpl.sprintfv('%%%ds', widths);
    ...
    function out = prettyprintStruct(s)
      ...
      if isscalar(s)
        ...
        out = fancyclip.internal.DispstrImpl.dispc(s);
      end
    ...
    function out = prettyprintTabular(t)
      ...
      if nVars == 0
        out = sprintf('%s table with zero variables', fancyclip.internal.DispstrImpl.size2str(size(t)));
        return;
      end
